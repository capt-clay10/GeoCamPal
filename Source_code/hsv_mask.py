...
class BBoxSelectorWindow(tk.Toplevel):
    def __init__(self, master, pil_image, callback, **kwargs):
        ...
        btn_zoom_in = tk.Button(bottom_frame, text="Zoom In", command=self.zoom_in)
        btn_zoom_in.pack(side="left", padx=5)
        btn_zoom_out = tk.Button(bottom_frame, text="Zoom Out", command=self.zoom_out)
        btn_zoom_out.pack(side="left", padx=5)
        btn_confirm = tk.Button(bottom_frame, text="Confirm BBox", command=self.confirm_bbox)
    ...
    def display_image(self):
        ...
        self.resized_image = self.original_pil_image.resize((new_w, new_h), Image.Resampling.LANCZOS)
    ...
    def confirm_bbox(self):
        if not self.bbox:
            messagebox.showwarning("BBox Selection", "No bounding box was selected.")
            return
        x, y, w, h = self.bbox
...
class HSVMaskTool(ctk.CTkToplevel):
    ...
    def __init__(self, master=None, mode="individual", *args, **kwargs):
        ...
        if self.mode in ("individual", "ml"):
            ...
            self.bind("<KP_Subtract>", lambda e: self.checkbox_invert_mask_toggle())
            ...
            self.image_label = ctk.CTkLabel(self.top_left_frame, text="", anchor="center")
            ...
            self.edge_label = ctk.CTkLabel(self.top_right_frame, text="", fg_color="white", anchor="center")
            ...
            self.filename_label = ctk.CTkLabel(main_frame, text="No file loaded", font=("Arial", 14))
            self.filename_label.pack(side="top", fill="x", pady=5)

            self.top_frame = ctk.CTkFrame(main_frame)
            self.top_frame.pack(side="top", fill="both", expand=True)

            self.progress_label = ctk.CTkLabel(self.top_frame, text="Ready for batch processing.")
            self.progress_label.pack(pady=10)

            self.batch_progress_bar = ctk.CTkProgressBar(self.top_frame, width=500)
            ...
            self.console_frame.pack(side="bottom", fill="x", expand=False, padx=5, pady=0)
            
            self.console_text = scrolledtext.ScrolledText(self.console_frame, wrap="word", height=6)
            self.console_text.pack(side="left", fill="both", expand=True)
        ...
        self.features = []  # list of (feature_type, [(x,y), (x,y), ...]) for polylines or polygons
    ...
    def update_edge_display(self, event=None):
        ...
        for (feature_type, points) in self.features:
            ...
            if feature_type == "polygon":
                ...
                cv2.polylines(overlay, [pts_np], isClosed=True, color=(0, 255, 0), thickness=2)
            else:
                # polyline
                thickness = int(self.edge_thickness_slider.get()) if hasattr(self, 'edge_thickness_slider') else 2
                cv2.polylines(overlay, [pts_np], isClosed=False, color=(0, 255, 0), thickness=thickness)
    ...
    def setup_controls(self, parent):
        ...
        self.use_inner_mask = tk.BooleanVar(master=self, value=False)
        self.inner_mask_check = ctk.CTkCheckBox(bbox_control_frame, text="Use Inner Mask", variable=self.use_inner_mask)
        ...
        self.enable_enhancements = tk.BooleanVar(master=self, value=True)
        enhance_chk = ctk.CTkCheckBox(enhance_frame, text="Enhance?", variable=self.enable_enhancements)
        enhance_chk.pack(side="left", padx=5)

        ctk.CTkLabel(enhance_frame, text="S Mult").pack(side="left", padx=2)
        self.s_multiplier_slider = ctk.CTkSlider(enhance_frame, from_=100, to=500)
        ...
        self.v_multiplier_slider = ctk.CTkSlider(enhance_frame, from_=100, to=500)
        ...
        self.h_low_slider, self.h_low_var, self.h_low_lbl = self.make_slider_with_label(first_lower_frame, "H", 0, 255, 0)
        self.s_low_slider, self.s_low_var, self.s_low_lbl = self.make_slider_with_label(first_lower_frame, "S", 0, 255, 0)
        self.v_low_slider, self.v_low_var, self.v_low_lbl = self.make_slider_with_label(first_lower_frame, "V", 0, 255, 0)
        ...
        self.h2_low_slider, self.h2_low_var, self.h2_low_lbl = self.make_slider_with_label(self.dual_lower_frame, "H2", 0, 255, 0)
        self.s2_low_slider, self.s2_low_var, self.s2_low_lbl = self.make_slider_with_label(self.dual_lower_frame, "S2", 0, 255, 0)
        self.v2_low_slider, self.v2_low_var, self.v2_low_lbl = self.make_slider_with_label(self.dual_lower_frame, "V2", 0, 255, 0)
        ...
        self.h_high_slider, self.h_high_var, self.h_high_lbl = self.make_slider_with_label(first_upper_frame, "H", 0, 255, 255)
        self.s_high_slider, self.s_high_var, self.s_high_lbl = self.make_slider_with_label(first_upper_frame, "S", 0, 255, 255)
        self.v_high_slider, self.v_high_var, self.v_high_lbl = self.make_slider_with_label(first_upper_frame, "V", 0, 255, 255)
        ...
        self.h2_high_slider, self.h2_high_var, self.h2_high_lbl = self.make_slider_with_label(self.dual_upper_frame, "H2", 0, 255, 255)
        self.s2_high_slider, self.s2_high_var, self.s2_high_lbl = self.make_slider_with_label(self.dual_upper_frame, "S2", 0, 255, 255)
        self.v2_high_slider, self.v2_high_var, self.v2_high_lbl = self.make_slider_with_label(self.dual_upper_frame, "V2", 0, 255, 255)
        ...
        if self.mode in ("ml", "individual"):
            ...
            btn_cut_feature = ctk.CTkButton(calc_frame, text="Edit Detected Feature", command=self.cut_detected_feature)
        ...
        if self.mode in ("individual", "ml"):
            btn_export_edge = ctk.CTkButton(export_buttons_frame, text="Export feature as training data", command=self.export_training_data)
            btn_export_edge.pack(side="left", padx=5)
            btn_export_mask = ctk.CTkButton(export_buttons_frame, text="Export mask as training data", command=self.export_mask_as_training_data)
            btn_export_mask.pack(side="left", padx=5)
            btn_export_test = ctk.CTkButton(export_buttons_frame, text="Export as Test Data", command=self.export_as_test_data)
            btn_export_test.pack(side="left", padx=5)
            if self.mode == "individual":
                btn_export_overlay = ctk.CTkButton(export_buttons_frame, text="Export as Overlay", command=self.export_as_overlay)
    ...
    def open_bbox_selector(self):
        if self.full_image is None:
            messagebox.showwarning("BBox Selector", "Please load an image first.")
            return
    ...
    def load_folder(self):
        ...
        self.current_index = 0
        if self.mode == "batch":
            self.filename_label.configure(text=f"Loaded {len(files)} images for batch")
        else:
    ...
    def compute_full_masks(self, original_image):
        if original_image.ndim == 3 and original_image.shape[2] == 4:
            ...
            full_alpha_mask = np.ones((original_image.shape[0], original_image.shape[1]), dtype=np.uint8)
        kernel_inner_full = np.ones((28, 28), np.uint8)
        self.full_alpha_mask = full_alpha_mask
        self.full_alpha_mask_inner = cv2.erode(full_alpha_mask, kernel_inner_full, iterations=1)
    ...
    def process_loaded_image(self, original_image):
        if original_image.ndim == 3 and original_image.shape[2] == 4:
            ...
            alpha = np.ones((img_bgr.shape[0], img_bgr.shape[1]), dtype=np.uint8)
        ...
        if self.scale < 1.0:
            ...
            self.cv_image = cv2.resize(img_bgr, new_size, interpolation=cv2.INTER_AREA)
            self.alpha_mask = cv2.resize(alpha, new_size, interpolation=cv2.INTER_NEAREST)
        else:
    ...
    def calculate_mask(self):
        ...
        if not self.use_bbox.get():
            ...
            if not bbox_text:
                ...
                try:
                    ...
                    messagebox.showerror("Error", f"Invalid bounding box format: {e}")
                    return
                x = int(x * self.scale)
                y = int(y * self.scale)
                w = int(w * self.scale)
                ...
                bbox_mask = cv2.bitwise_and(bbox_mask, (self.alpha_mask * 255).astype(np.uint8))
        ...
        masked_bgr = cv2.bitwise_and(self.cv_image, self.cv_image, mask=bbox_mask)
        ...
        if self.enable_enhancements.get():
            ...
            s_chan = np.clip(s_chan.astype(np.float32) * s_mult, 0, 255).astype(np.uint8)
            v_chan = np.clip(v_chan.astype(np.float32) * v_mult, 0, 255).astype(np.uint8)
        ...
        mask1 = cv2.inRange(enhanced_hsv, (h_low, s_low, v_low), (h_high, s_high, v_high))
        ...
        if self.use_dual_hsv.get():
            ...
            mask2 = cv2.inRange(enhanced_hsv, (h2_low, s2_low, v2_low), (h2_high, s2_high, v2_high))
    ...
    def calculate_edge(self):
        ...
        if self.advanced_check_var.get():
            ...
            try:
                ...
                messagebox.showerror("Error", "Invalid min or max contour size. Please enter integers.")
                return
            if max_size < min_size:
                messagebox.showerror("Error", "Maximum contour size must be >= minimum.")
                return
            filtered_contours = []
            for cnt in contours:
            ...
            if not filtered_contours:
                messagebox.showwarning("No Contours", "No contours found in size range.")
                return
            largest = max(filtered_contours, key=lambda c: cv2.arcLength(c, True))
        else:
        ...
        if self.mode != "batch":
            ...
            if len(valid_points) > 1:
                ...
                cv2.polylines(overlay, [pts], isClosed=False, color=(0, 255, 0), thickness=thickness)
            disp_overlay = cv2.resize(overlay, (self.cv_image.shape[1], self.cv_image.shape[0]), interpolation=cv2.INTER_AREA)
    ...
    def cut_detected_feature(self):
        ...
        The user can add new polylines or polygons. By default,
        we treat the detected shape as a polyline for editing.
        """
        ...
        v_scroll = tk.Scrollbar(self.edit_canvas_container, orient="vertical", command=self.edit_canvas.yview)
        v_scroll.grid(row=0, column=1, sticky="ns")
        h_scroll = tk.Scrollbar(self.edit_canvas_container, orient="horizontal", command=self.edit_canvas.xview)
        h_scroll.grid(row=1, column=0, sticky="ew")
        ...
        btn_delete_all = ctk.CTkButton(mode_frame, text="Delete All Vertices",
                                       command=self.delete_all_vertices, fg_color="red",
                                       text_color="white")
        
        btn_delete_all.pack(side="left", padx=5)

        # "Create New Edge"
        btn_create_edge = ctk.CTkButton(mode_frame, text="Create New Edge", command=self.create_new_edge)
        btn_create_edge.pack(side="left", padx=5)

        # "Create Polygon"
        btn_create_polygon = ctk.CTkButton(mode_frame, text="Create Polygon", command=self.create_new_polygon)
        btn_create_polygon.pack(side="left", padx=5)

        btn_frame = ctk.CTkFrame(self.control_frame)
        btn_frame.pack(side="top", pady=5)

        self.undo_btn = ctk.CTkButton(btn_frame, text="Undo", command=self.undo_last_action)
        self.undo_btn.pack(side="left", padx=5)
        self.reset_btn = ctk.CTkButton(btn_frame, text="Reset", command=self.reset_to_initial)
        ...
        ctk.CTkButton(btn_frame, text="Zoom In", command=lambda: self.adjust_zoom(1.2)).pack(side="left", padx=2)
        ctk.CTkButton(btn_frame, text="Zoom Out", command=lambda: self.adjust_zoom(0.8)).pack(side="left", padx=2)
        ctk.CTkButton(btn_frame, text="Reset View", command=self.reset_view).pack(side="left", padx=2)
    ...
    def redraw_canvas(self, event=None):
        ...
        resized_img = pil_img.resize((scaled_width, scaled_height), Image.LANCZOS)
        self.zoomed_image = ImageTk.PhotoImage(resized_img)

        self.edit_canvas.create_image(0, 0, anchor=tk.NW, image=self.zoomed_image)
        self.edit_canvas.config(scrollregion=(0, 0, scaled_width, scaled_height))
    ...
    def on_canvas_single_click(self, event):
        ...
        if self.is_polygon_mode and len(self.edited_edge_points) > 2:
            ...
            if self.distance(x_img, y_img, first_pt[0], first_pt[1]) < 10:
                ...
                print("Polygon closed and stored. You can make another polygon if you wish.")
                self.edited_edge_points = []
                self.edit_history = []
                self.selected_vertex = None
                self.redraw_canvas()
    ...
    def batch_process(self):
        ...
        for i, file_path in enumerate(self.image_files):
            ...
            self.full_image = original_image.copy()
            self.compute_full_masks(original_image)
            self.cv_image, self.alpha_mask, self.scale = self.prepare_cv_image_for_batch(original_image)
            self.calculate_mask()
            self.calculate_edge()

            base_name = os.path.basename(file_path)
            out_geojson = os.path.join(geojson_folder, os.path.splitext(base_name)[0] + ".geojson")
            out_overlay = os.path.join(overlay_folder, os.path.splitext(base_name)[0] + "_overlay.png")
            if len(self.edge_points) >= 2:
                ...
                try:
                    ...
                    pass
                real_world_coords = []
                for px, py in self.edge_points:
                    if transform is not None:
                        lon, lat = transform * (px, py)
                        real_world_coords.append((lon, lat))
                    else:
                        real_world_coords.append((px, py))
                if len(real_world_coords) > 1:
                    line = LineString(real_world_coords)
                    if crs is not None:
                        gdf = gpd.GeoDataFrame(index=[0], geometry=[line], crs=crs)
                    else:
            ...
            if self.mode == "batch":
                ...
                self.batch_progress_bar.set(fraction)
                self.progress_label.configure(text=f"Processed {i+1} of {total_files}")
                self.update_idletasks()

        if self.mode == "batch":
            self.progress_label.configure(text="Batch processing complete.")
        messagebox.showinfo("Batch Process",
                            f"Processed {processed_count} images.\nGeoJSON => {geojson_folder}\nOverlay => {overlay_folder}")
    ...
    def prepare_cv_image_for_batch(self, original_image):
        ...
        if scale < 1.0:
            ...
            small_bgr = cv2.resize(img_bgr, new_size, interpolation=cv2.INTER_AREA)
            small_alpha = cv2.resize(alpha, new_size, interpolation=cv2.INTER_NEAREST)
        else:
            small_bgr = img_bgr
            small_alpha = alpha
    ...
    def export_training_data(self):
        """
        Exports all confirmed features (polylines/polygons) from self.features into:
          - A merged mask (filled polygons, polylines drawn)
          - An overlay PNG (same shapes in green on the original image)
          - A GeoJSON file (all shapes combined)
          - A COCO JSON annotation file (single category), using:
              feature_id_entry => category name
              image_id_entry   => numeric image_id
              category_id_entry => numeric category_id
    
        If self.features is empty, we assume the user never clicked "Confirm Feature"
        or created shapes. We skip exporting to avoid an empty dataset.
        """
        # 1) Read user input
        ...
        image_id_str = self.image_id_entry.get().strip()
        category_id_str = self.category_id_entry.get().strip()
        try:
        ...
        # 2) Check if we actually have an image
        if not isinstance(self.full_image, np.ndarray):
            messagebox.showerror("Error", "No full image is loaded or invalid image data.")
            return
    
        # 3) Check we have at least one confirmed feature
        ...
        # 5) Copy original image to images_folder
        base_name = os.path.basename(self.image_path)
        dest_image_path = os.path.join(images_folder, base_name)
        shutil.copy2(self.image_path, dest_image_path)
    
        # 6) Build a merged mask & overlay
        ...
        # 7) Prepare data structures for COCO + GeoJSON
        coco_annotations = []
        annotation_id = 1
        transform = None
        crs = None
        ...
        shape_list = []  # for storing shapely geometry objects
    
        # 8) Loop through each feature in self.features
        for (feature_type, pts) in self.features:
            if len(pts) < 2:
                # skip any shape with < 2 points
                continue
    
            pts_np = np.array(pts, dtype=np.int32).reshape((-1, 1, 2))
    
            # If polygon, ensure it's closed for COCO segmentation + mask fill
            polygon_pts = pts[:]
            is_polygon = (feature_type == "polygon")
            if is_polygon and polygon_pts[0] != polygon_pts[-1]:
                polygon_pts.append(polygon_pts[0])
    
            # Build segmentation for COCO
            segmentation = []
            ...
            # Build geometry (geojson)
            if transform is not None:
                real_coords = []
                for (px, py) in pts:
                    lon, lat = transform * (px, py)
                    real_coords.append((lon, lat))
                shape_obj = Polygon(real_coords) if is_polygon else LineString(real_coords)
            else:
                shape_obj = Polygon(pts) if is_polygon else LineString(pts)
        ...
        # 11) Save GeoJSON (if we have shapes)
        if shape_list:
            gdf = gpd.GeoDataFrame(geometry=shape_list, crs=crs)
            geojson_filename = os.path.splitext(base_name)[0] + ".geojson"
        ...
        coco_json_path = os.path.join(coco_folder, os.path.splitext(base_name)[0] + ".json")
        ...
        messagebox.showinfo(
            ...
            f"- COCO => {coco_json_path}\n"
            f"- GeoJSON => {geojson_path if shape_list else '(no shapes)'}"
        )
    ...
    def export_as_test_data(self):
        ...
        export_path = self.export_path_entry.get().strip()
        if not export_path:
            messagebox.showerror("Error", "Please specify a path to save files.")
            return
        ...
        messagebox.showinfo("Export Test Data", f"Test image exported to:\n{dest_path}")
    ...
    def export_as_overlay(self):
        ...
        pts = np.array(self.edge_points, dtype=np.int32).reshape((-1, 1, 2))
        cv2.polylines(overlay, [pts], isClosed=False, color=(0, 255, 0), thickness=thickness)
        out_overlay = os.path.join(export_path, os.path.splitext(base_name)[0] + "_overlay.png")
        cv2.imwrite(out_overlay, overlay)
        messagebox.showinfo("Export Overlay", f"Overlay saved to:\n{out_overlay}")
    ...
    def f6_action(self):
        ...
        if not isinstance(self.full_image, np.ndarray):
            ...
            return
        self.process_loaded_image(self.full_image)
        self.do_invert_mask.set(True)
        self.calculate_mask()
        self.calculate_edge()
        self.do_invert_mask.set(False)
    ...
    def load_settings(self):
        ...
        self.advanced_check_var.set(data.get("advanced_settings_enabled", False))
