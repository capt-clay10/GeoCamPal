...
class CreateHomographyMatrixWindow(ctk.CTkToplevel):
    def __init__(self, master=None, *args, **kwargs):
        ...
        self.label_note = ctk.CTkLabel(
            ...
            fg_color="white",        # solid white background
            text_color="black",      # black text
            corner_radius=0          # remove any rounded corners if you like
        )
        ...
        self.chk_exclude = ctk.CTkCheckBox(
            self.panel_exclude, text="Exclude GCPs", variable=self.exclude_var)
        self.chk_exclude.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        self.label_exclude = ctk.CTkLabel(
            self.panel_exclude, text="(List of integer indices, comma separated)")
        ...
        self.btn_compute = ctk.CTkButton(
            self.panel3, text="Compute Matrix", command=self.compute_matrix)
        self.btn_compute.grid(row=0, column=0, padx=5, pady=5)

        self.btn_compute_accuracy = ctk.CTkButton(
            self.panel3, text="Compute Accuracy", command=self.compute_accuracy)
        ...
        self.lbl_adv_info = ctk.CTkLabel(
            self.panel_adv, text="", fg_color="transparent")
        ...
        self.adv_buttons_frame.grid(
            row=len(adv_options), column=0, columnspan=2, pady=10)
    ...
    def toggle_advanced(self):
        ...
        if self.advanced_var.get():
            self.lbl_adv_info.configure(
                text="Using simulated annealing to identify best subset of GCPS")
    ...
    def compute_matrix(self):
        ...
        if self.exclude_var.get():
            ...
            if exclude_str.strip():
                try:
                    exclude_list = [int(x.strip()) for x in exclude_str.split(
                        ',') if x.strip() != ""]
        ...
        pixel_points = gcp_data[['Pixel_X', 'Pixel_Y']
        ...
        H_norm, status = cv2.findHomography(
            pixel_points_norm_2d, utm_points_norm_2d, cv2.RANSAC, 0.5)
        if H_norm is None:
            self.log(
                "Failed to compute homography matrix. Please check your input data.")
            messagebox.showerror(
                "Error", "Failed to compute homography matrix. Please check your input data.")
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Error saving homography matrix: {e}")
            return
        ...
        messagebox.showinfo(
            "Success", "Homography matrix computed and saved successfully.")
    ...
    def compute_accuracy(self):
        ...
        if self.H_final is None or self.pixel_points is None or self.utm_points is None:
            messagebox.showerror(
                "Error", "Please compute the homography matrix first!")
            return
        ...
        utm_estimated = cv2.perspectiveTransform(
            pixel_points_reshaped, self.H_final)
        ...
        utm_estimated_manual = transformed[:,
                                           :2] / transformed[:, 2][:, np.newaxis]
    ...
    def compute_homography_and_errors(self, pixel_points, utm_points, subset_indices, ransac_thresh=0.5):
        ...
        try:
            ...
            H_norm, _ = cv2.findHomography(
                px_norm_2d, utm_norm_2d, cv2.RANSAC, ransac_thresh)
            if H_norm is None:
                return 9999.0, 9999.0, 9999.0, 9999.0, 0.0, 9999.0, None
    ...
    def run_simulated_annealing(self):
        ...
        threading.Thread(
            target=self.run_simulated_annealing_thread, daemon=True).start()
    ...
    def run_simulated_annealing_thread(self):
        ...
        if self.pixel_points is None or self.utm_points is None:
            self.thread_log(
                "Error: Please compute the homography matrix first (or load a valid GCP file).")
            return
        ...
        if n_subset > n_gcp:
            self.thread_log(
                "Error: Number of GCPs for subset cannot exceed total GCP count.")
            return
        ...
        current_subset = np.random.choice(
            all_indices, size=n_subset, replace=False)
        (sub_mean, sub_median, sub_std, sub_max, sub_inlier_ratio, sub_count_above_5, H_candidate) = \
            self.compute_homography_and_errors(
                self.pixel_points, self.utm_points, current_subset, ransac_thresh)
        full_metrics = self.compute_full_errors(
            H_candidate, self.pixel_points, self.utm_points)
        ...
        for iteration in range(max_iterations):
            ...
            (sub_mean2, sub_median2, sub_std2, sub_max2, sub_inlier_ratio2, sub_count_above_5_2, H_candidate2) = \
                self.compute_homography_and_errors(
                    self.pixel_points, self.utm_points, new_subset, ransac_thresh)
            full_metrics2 = self.compute_full_errors(
                H_candidate2, self.pixel_points, self.utm_points)
            ...
            if cost_diff < 0:
                ...
                accept_prob = np.exp(-cost_diff /
                                     temperature) if temperature > 0 else 0.0
            ...
            if (iteration + 1) % 50000 == 0:
                self.thread_log(f"Iteration {
                                iteration+1}/{max_iterations} | Temp={temperature:.4f} | BestCost={best_cost:.3f}")

        elapsed = time.time() - start_time
        self.thread_log(f"Simulated annealing finished in {
                        elapsed:.2f} seconds")
        self.thread_log(f"Best cost: {best_cost:.3f}")
        full_metrics_best = self.compute_full_errors(
    ...
    def accept_and_export_new_matrix(self):
        ...
        if self.best_H_annealing is None:
            messagebox.showerror(
                "Error", "No simulated annealing result found. Run the search first.")
            return
        output_name = self.entry_output_name.get().strip()
        if not output_name:
            messagebox.showerror("Error", "Please enter an output file name.")
