...
class GeoReferenceModule(ctk.CTk):
    ...
    def initialize_components(self):
        ...
        for text, value in aoi_options:
            rb = ctk.CTkRadioButton(self.aoi_panel, text=text, variable=self.aoi_var, value=value)
        ...
        for label in crop_labels:
            ctk.CTkLabel(self.crop_panel, text=label).grid(row=0, column=col, padx=5)
            col += 1
    ...
    def load_homography(self):
        ...
        if path:
            try:
                ...
                messagebox.showerror("Error", f"Invalid homography matrix: {str(e)}")
    ...
    def warp_image(self, img, original_corners):
        ...
        warped = cv2.warpPerspective(img, H_final, (output_width, output_height), flags=cv2.INTER_LANCZOS4)
        ...
        def process():
            ...
            for i, path in enumerate(self.image_list):
                try:
                    ...
                    output_path = os.path.join(self.output_folder, f"{base_name}.tif")
    ...
    def georeference_and_save_image(self, img_path, output_path):
        ...
        adjusted_corners = cv2.perspectiveTransform(original_corners, H_final).reshape(-1, 2)
        gcps = []
        ...
        try:
            ...
            messagebox.showerror("Error", "Invalid EPSG code provided. Please enter a valid EPSG number.")
            return False
        ...
        dst_ds = driver.Create(output_path, warped.shape[1], warped.shape[0], 4, gdal.GDT_Byte, options=["ALPHA=YES"])
